...
def test_simplify_geom_repr():
    ...
    polygon_wkt = "POLYGON ((0.000000 0.000000, 1.000000 0.000000, 1.000000 1.000000, 0.000000 1.000000, 0.000000 0.000000))"
    expected = "POLYGON(...)"
    assert simplify_geom_repr(polygon_wkt) == expected

def test_inspect_processor_initialization(sample_gdf):
...
def test_head_operation(inspect_processor, sample_gdf):
    """Test head operation."""
    # Test default head (5 rows)
    result = inspect_processor.head()
    assert isinstance(result, pd.DataFrame)  # Result is a DataFrame with WKT geometries
    assert len(result) == len(sample_gdf)  # Since sample has fewer rows than default
    assert 'geometry' in result.columns
    assert isinstance(result['geometry'].iloc[0], str)  # Geometry should be WKT string
...
def test_tail_operation(inspect_processor, sample_gdf):
    """Test tail operation."""
...
def test_get_crs(inspect_processor):
    """Test CRS retrieval."""
...
def test_get_shape(inspect_processor, sample_gdf):
...
def test_get_dtypes(inspect_processor):
